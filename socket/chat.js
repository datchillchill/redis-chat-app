...
function initializeChat(io, redis, pubClient, subClient, helpers) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send private message', async ({ receiverUserId, text, replyTo, file }) => {
            try {
                    ...
                    if (originalMessageString) {
                        ...
                        if (originalUser && (originalMessage.text || originalMessage.file)) {
                            ...
                            replyToObject = {
                                ...
                                user: originalUser,
                                text: previewText.substring(0, 75) + (previewText.length > 75 ? '...' : '')
                            };
                        }
                    }
                }
